...
node tools/generate_audio_pack.mjs
```

The script reads the key from `.env` (or an exported `ELEVENLABS_API_KEY` environment variable) and writes base64 mp3 clips into the `AUDIO_PACK` block of `index.html`. `.env` is gitignored, and the key is never stored anywhere else. Useful flags: `--voice <id>` to pick the announcer voice, `--only voice,sfx,music` to regenerate a subset. Any clip that is missing or fails to decode silently falls back to the procedural synth, so a partial pack is fine.
